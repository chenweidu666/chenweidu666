...
import { ClothesFormData, Clothes } from '../types';
...
const EditClothes = () => {
  ...
  const onSubmit = async (data: ClothesFormData) => {
    ...
    try {
      ...
      await clothesApi.update(parseInt(id), data);
      toast.success('衣服更新成功！');
      navigate(`/clothes/${id}`);
    } catch (error) {
  ...
  return (
    <div className="max-w-2xl mx-auto">
      <div className="flex items-center space-x-4 mb-8">
        ...
        <h1 className="text-3xl font-bold text-gray-900">编辑衣服</h1>
      </div>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
        ...
        <div className="flex justify-end space-x-4">
          ...
          >
            <Save size={20} />
            <span>{isSubmitting ? '更新中...' : '更新衣服'}</span>
          </button>
        </div>
      </form>
    </div>
